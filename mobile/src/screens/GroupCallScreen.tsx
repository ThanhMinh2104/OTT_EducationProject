...
const SelectMembersScreen = ({
  ...
  members: GroupMember[];
  onCall: (selected: GroupMember[]) => void;
  onCancel: () => void;
}) => {
      ...
      {selected.length > 0 && (
        <View style={styles.selectedBar}>
          <Text style={styles.selectedBarText}>
            Đã chọn: {selected.map((s) => s.name).join(", ")}
          </Text>
        </View>
      )}
    </View>
...
const GroupCallScreen = ({
  ...
  const setupListeners = useCallback(() => {
    ...
    socket.on(
      ...
      (data: { groupID: string; userID: string }) => {
        ...
        setParticipants((prev) =>
          prev.map((p) =>
            p.userID === data.userID ? { ...p, status: "left" as const } : p,
          ),
        );
      },
    );
  ...
  const startCall = useCallback(
    (selected: GroupMember[]) => {
      ...
      socket.emit("group-call-start", {
        ...
        invitedUserInfos: selected.map((m) => ({
          userID: m.userID,
          name: m.name,
          avatar: m.avatar,
        })),
  ...
  const addMembersToCall = useCallback(
    (selectedMembers: GroupMember[]) => {
      ...
      socket.emit("group-call-add-members", {
        ...
        inviterInfo: { name: currentUser.name, avatar: currentUser.anhDaiDien },
  ...
  const availableMembers = members.filter(
    (m) =>
      ...
      !participants.some((p) => p.userID === m.userID),
  );
      ...
      <View style={styles.callContainer}>
        ...
        <View style={styles.callHeader}>
          <View style={styles.callHeaderLeft}>
            ...
            {screen === "in-call" && availableMembers.length > 0 && (
              <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ParticipantTile
              participant={item}
              isLocal={item.userID === currentUser.userID}
            />
          )}
        />
