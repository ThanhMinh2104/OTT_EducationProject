...
const ChatScreenEnhanced = ({
  ...
  useEffect(() => {
    ...
    socket.on("ChatByUserID", (data: Chat[]) => {
      ...
      console.log(
        `✅ Loaded ${friendChats.length} friend chats + ${strangers.length} stranger chats`,
      );
    ...
    socket.on("newChat1-1", (newChat: Chat) => {
      ...
      if (newChat.isStranger) {
        ...
        setStrangerChats((prev) => {
          ...
          return [newChat, ...prev].sort((a, b) => {
            ...
            const bT = b.lastMessage?.slice(-1)[0]?.timestamp || 0;
            return (
              new Date(bT as string).getTime() -
              new Date(aT as string).getTime()
            );
          });
        });
        ...
        setStrangerChats((prev) =>
          prev.filter((c) => c.chatID !== newChat.chatID),
      ...
      if (newChat.type === "private") {
        const fetchMemberInfo = async () => {
          ...
          if (!stored) return;
          const me = JSON.parse(stored);
          const otherId = newChat.members.find(
    ...
    socket.on(
      ...
      async (data: { userID: string; friendID: string }) => {
          ...
          if (chatWithFriend) {
            ...
            setChats((prevChats) => {
              const exists = prevChats.find(
                (c) => c.chatID === chatWithFriend.chatID,
              );
              if (exists) return prevChats;
              return [
                { ...chatWithFriend, isStranger: false },
                ...prevChats,
              ].sort((a, b) => {
                const aT = a.lastMessage?.slice(-1)[0]?.timestamp || 0;
                const bT = b.lastMessage?.slice(-1)[0]?.timestamp || 0;
          ...
          setChats((prev) => {
            ...
            if (chatWithBlocker) {
              ...
              setStrangerChats((prevStrangers) => {
                const exists = prevStrangers.find(
                  (c) => c.chatID === chatWithBlocker.chatID,
                );
                if (exists) return prevStrangers;
                return [
                  { ...chatWithBlocker, isStranger: true },
                  ...prevStrangers,
                ].sort((a, b) => {
                  const aT = a.lastMessage?.slice(-1)[0]?.timestamp || 0;
                  const bT = b.lastMessage?.slice(-1)[0]?.timestamp || 0;
  ...
  useEffect(() => {
    ...
    const onUserMentioned = (data: any) => {
      ...
      if (selectedChat?.chatID === data.groupID) {
        Alert.alert(
          "Nhắc tên",
          `${data.mentionerName} đã nhắc tên bạn trong nhóm`,
        );
      }
    };
    ...
    const onGroupMentionAll = (data: any) => {
      ...
      if (
        selectedChat?.chatID === data.groupID &&
        data.mentionerID !== user.userID
      ) {
        Alert.alert(
          "Nhắc tên cả nhóm",
          `${data.mentionerName} đã nhắc tên tất cả mọi người`,
        );
      }
    };
  ...
  const handleDeleteChat = async () => {
    ...
    try {
      ...
      setStrangerChats((prev) =>
        prev.filter((c) => c.chatID !== deletingChat.chatID),
      );
    } catch {
      /* ignore */
    } finally {
  ...
  const handleUnfriend = async () => {
    ...
    const otherId = selectedChat.members.find(
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/contacts/friend/${otherId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      if (response.ok) {
        ...
        setChats((prev) =>
          prev.filter((c) => c.chatID !== selectedChat.chatID),
        );
        setStrangerChats((prev) => {
        ...
        setSelectedChat((prev) =>
          prev ? { ...prev, isStranger: true } : prev,
        );
      }
    } catch (err) {
  ...
  const handleAcceptFriendRequest = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/contacts/accept-friend-request`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        setSelectedChat((prev) =>
          prev ? { ...prev, isStranger: false } : prev,
        );
  ...
  const handleCancelFriendRequest = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/contacts/cancel-friend-request`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUserProfileById = async (targetUserID: string) => {
    if (targetUserID === "bot") {
      setOtherProfile({
        ...
        sdt: "1900-BOT",
        anhDaiDien:
          "https://res.cloudinary.com/dgqppqcbd/image/upload/v1727405200/ai-bot-avatar.png",
        anhBia:
          "https://res.cloudinary.com/ddu7vms87/image/upload/v1740316684/p79itfnd9o7atd62269y.jpg",
  ...
  useEffect(() => {
          ...
          if (chatExists) {
            ...
            const updated = prev.map((c) => {
              ...
              const exists = msgs.find(
                (m) => m.messageID === msg.messageID || m.tempID === msg.tempID,
              );
              const newMsgs = exists
                ? msgs.map((m) =>
                    m.messageID === msg.messageID || m.tempID === msg.tempID
                      ? { ...m, ...msg }
                      : m,
                  )
                : [...msgs, msg];
    ...
    const onNewGroupMessage = (msg: any) => {
      ...
      setChats((prev) =>
        prev.map((c) => {
          ...
          const msgs = c.lastMessage || [];
          const exists = msgs.find(
            (m) => m.messageID === normalizedMsg.messageID,
          );
    ...
    const onTypingStart = ({
      ...
      setTypingUsers((prev) =>
        prev.find((u) => u.userID === uid)
          ? prev
          : [...prev, { userID: uid, userName }],
      );
    };
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.messageID === updated.messageID
              ? { ...m, pinnedInfo: undefined }
              : m,
          ),
        );
        setPinnedMessages((prev) =>
          prev.filter((m) => m.messageID !== updated.messageID),
        );
      }
    };
  ...
  const handleSelectChat = (chat: Chat) => {
    ...
    const chatInList =
      ...
      strangerChats.find((c) => c.chatID === chat.chatID);
    if (!chatInList) {
      if (chat.isStranger) {
    ...
    const normalizedCached =
      chat.type === "group"
        ? cachedMessages.map((m: any) => ({
            ...m,
            chatID: m.groupID || chat.chatID,
          }))
        : cachedMessages;
    setSelectedChat(chat);
    setMessages(normalizedCached);
    setPinnedMessages(
      normalizedCached.filter((m: any) => m.pinnedInfo?.pinnedBy),
    );
    setReplyTo(null);
    setInputText("");
    setPage(1);
      ...
      try {
        ...
        if (chat.type === "group") {
          ...
          const res = await fetch(
            `${API_URL}/api/groups/${groupID}/messages?page=1&limit=50`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          ...
          if (data.messages && Array.isArray(data.messages)) {
            ...
            setSelectedChat((current) => {
              if (current?.chatID === chat.chatID) {
                ...
                setPinnedMessages(
                  normalized.filter((m: any) => m.pinnedInfo?.pinnedBy),
                );
              }
              return current;
            });
          ...
          if (messageArray && Array.isArray(messageArray)) {
            ...
            setSelectedChat((current) => {
              if (current?.chatID === chat.chatID) {
                setMessages(deduped);
                setPinnedMessages(
                  deduped.filter((m: any) => m.pinnedInfo?.pinnedBy),
                );
                setHasMore(
                  Array.isArray(data)
                    ? false
      ...
      if (chat.type === "group") {
        try {
          ...
          const settingsRes = await fetch(
            `${API_URL}/api/groups/${chat.chatID}/settings`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          if (settingsRes.ok) {
            ...
            setCanEditGroupInfo(
              isOwnerOrAdmin || (perms?.changeNameAvatar ?? true),
            );
          }
        } catch {
  ...
  const loadMoreMessages = async () => {
    ...
    try {
      ...
      if (selectedChat.type === "group") {
        const res = await fetch(
          `${API_URL}/api/groups/${selectedChat.chatID}/messages?page=${nextPage}&limit=50`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        const data = await res.json();
        if (data.messages) {
          newMessages = data.messages.map((m: any) => ({
            ...m,
            chatID: m.groupID || selectedChat.chatID,
          }));
        ...
        hasMoreData = Array.isArray(data)
          ? false
          : data.page * data.limit < data.total;
      }
        ...
        (ack: any) => {
          ...
          if (ack?.success && ack?.message) {
            ...
            setMessages((prev) => {
              ...
              const alreadyExists = withoutTemp.find(
                (m) => m.messageID === realMsg.messageID,
              );
  ...
  const uploadFiles = async (
    ...
    try {
      ...
      if (data.urls?.length > 0) {
        ...
        const groupId =
          msgType === "image" && data.urls.length > 1
  ...
  const sendAudio = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Lỗi",
        `Không thể gửi audio: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setIsUploading(false);
    }
  ...
  const handleStickerSelect = (url: string) => {
    if (!selectedChat || !user) return;
    dispatchMessageContent(
      { content: "", type: "sticker", media_url: [url] },
      replyTo || undefined,
    );
    setShowEmoji(false);
    setReplyTo(null);
  ...
  const handleGifSelect = (url: string) => {
    if (!selectedChat || !user) return;
    dispatchMessageContent(
      { content: "", type: "gif", media_url: [url] },
      replyTo || undefined,
    );
    setShowEmoji(false);
    setReplyTo(null);
  ...
  const handleDeleteLocal = (msg: Message) => {
    ...
    if (msg.type === "image" && msg.groupId) {
      ...
      console.log(
        `📸 Deleting ${messagesToDelete.length} images from group ${msg.groupId}`,
      );
    }
  ...
  const handleForwardSubmit = async () => {
    ...
    try {
      console.log(
        ...
        selectedChatsForForward.length,
        "chats",
      );
      ...
      Alert.alert(
        "Thành công",
        `Đã chuyển tiếp tin nhắn đến ${selectedChatsForForward.length} cuộc trò chuyện`,
      );
  ...
  const renderMessage = (item: Message) => {
    ...
    if (isNotification) {
      ...
      if (content.startsWith("##FRIENDSHIP##")) {
        ...
        return (
          <View
            key={item.messageID || item.tempID}
          ...
          >
            <View style={styles.notifBadge}>
              <Text style={styles.notifText}>
                  ...
                  onPress={async () => {
                    try {
                      ...
                      const [userRes, statusRes] = await Promise.all([
                        ...
                        fetch(
                          `${API_URL}/api/contacts/friend-status/${friendID}`,
                          {
                            headers: { Authorization: `Bearer ${token}` },
                          },
                        ),
                ...
                đã trở thành bạn bè.
                {isNew ? " Hãy bắt đầu cuộc trò chuyện." : ""}
              </Text>
            </View>
          </View>
        );
    ...
    if (item.type === "group-call") {
      return (
        <View
          key={item.messageID || item.tempID}
          style={styles.groupCallMsgWrapper}
        >
          <TouchableOpacity
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.groupCallMsgSub}>
                {item.senderInfo?.name} đã bắt đầu
              </Text>
            </View>
        ...
        return (
          <View>
            ...
            <Text
              style={[
                styles.messageTime,
                isMine ? styles.timeOnMedia : styles.timeOnMediaOther,
              ]}
            >
              {timeStr}
            </Text>
      ...
      if (item.type === "sticker" && item.media_url?.[0]) {
        return (
          ...
          >
            ...
            />
            <Text style={[styles.messageTime, styles.timeOnMediaOther]}>
              {timeStr}
            </Text>
          </TouchableOpacity>
        );
      ...
      if (item.type === "file" && item.media_url?.[0]) {
        ...
        return (
          <TouchableOpacity
            style={[
              styles.fileCard,
              isMine ? styles.fileCardMine : styles.fileCardOther,
            ]}
            onPress={() => {
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={[
                  styles.fileCardName,
                  isMine ? styles.fileCardNameMine : styles.fileCardNameOther,
                ]}
                numberOfLines={2}
              >
                {fileName}
              ...
              <Text
                style={[
                  styles.fileCardSub,
                  isMine ? styles.fileCardSubMine : styles.fileCardSubOther,
                ]}
              >
                Nhấn để tải xuống
              </Text>
          ...
          {item.replyTo && (
            <View
              style={[
                ...
                isMine ? styles.replyInBubbleMine : styles.replyInBubbleOther,
              ]}
            >
              <Text
                style={[
    ...
    return (
      ...
      >
        ...
        {!isMine && (
          <TouchableOpacity
            onPress={() => handleUserProfileById(item.senderID)}
          >
            <Image
              source={{
                uri:
        ...
        <View
          style={[
            styles.msgContent,
            isMine ? styles.msgContentMine : styles.msgContentOther,
          ]}
        >
          {renderBubbleContent()}
        </View>
  ...
  const renderMessageGroup = (
    group: import("../utils/messageGrouping").MessageGroup,
  ) => {
    const isMine = group.senderID === user?.userID;
    const firstMsg = group.messages[0] as Message; // ⭐ Cast to local Message type
    const timeStr = new Date(group.timestamp).toLocaleTimeString("vi-VN", {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    return (
      ...
      >
        ...
        {!isMine && (
          <TouchableOpacity
            onPress={() => handleUserProfileById(group.senderID)}
          >
            <Image
              source={{
                uri:
      ...
      <View style={styles.container}>
                ...
                <View style={styles.strangerAvatarStack}>
                  {strangerChats.slice(0, 3).map((c, i) => (
                    <Image
                      ...
                      style={[
                        styles.strangerStackAvatar,
                        { left: i * 16, zIndex: 3 - i },
                      ]}
                    />
                  ))}
                </View>
        ...
        >
          <View style={styles.strangerScreen}>
            {/* Header */}
            <View
              style={[styles.strangerHeader, { paddingTop: insets.top + 6 }]}
            >
              <TouchableOpacity
                onPress={() => setShowStrangerInbox(false)}
                style={styles.backButton}
              >
                <Ionicons name="chevron-back" size={26} color="#fff" />
              </TouchableOpacity>
              <Text style={styles.strangerHeaderTitle}>
  ...
  const renderMentionedText = (
    ...
    const hasSpecial = ["@all", "@bot", "@gif", "@sticker"].some((s) =>
      lowerContent.includes(s),
    );
    const hasMentions =
      (messageMentions && messageMentions.length > 0) || hasSpecial;
    ...
    const memberPatterns = sortedMembers
      ...
      >
        {parts.map((part, index) => {
          if (part.startsWith("@")) {
            ...
            const mentionMember = sortedMembers.find(
              (u) =>
                u.name.toLowerCase().trim() === candidate ||
                (messageMentions &&
                  messageMentions.includes(u.userID) &&
                  u.name.toLowerCase().trim().includes(candidate)),
            );

            const isValid =
              isAll ||
              ["bot", "gif", "sticker"].includes(candidate) ||
              !!mentionMember;
    ...
    <View style={styles.container}>
              ...
              style={[
                styles.statusDot,
                {
                  backgroundColor: (() => {
                    ...
                    return otherUser?.trangThai === "online"
                      ? "#22c55e"
                      : "#9ca3af";
                  })(),
                },
                ...
                onPress={async () => {
                  try {
                    ...
                    const [userRes, statusRes] = await Promise.all([
                      ...
                      fetch(
                        `${API_URL}/api/contacts/friend-status/${otherId}`,
                        {
                          headers: { Authorization: `Bearer ${token}` },
                        },
                      ),
                    ...
                    if (userRes.ok && statusRes.ok) {
                      ...
                      const target = {
                        ...userData,
                        friendStatus: statusData.status,
                      };
                ...
                >
                  ...
                  >
                    ...
                    <View style={styles.pinnedBannerItemText}>
                      ...
                      <Text
                        style={styles.pinnedBannerItemMessage}
                        numberOfLines={1}
                      >
                        {item.content || "[Media]"}
                      </Text>
                    ...
                    <View
                      style={[
                        ...
                        idx >= pinnedMessages.length - 1 &&
                          styles.pinnedBannerDropdownTop,
                      ]}
                    >
                      <TouchableOpacity
                      ...
                      >
                        <Ionicons
                          ...
                          color="#333"
                        />
                        <Text style={styles.pinnedBannerDropdownText}>
                          Đưa lên đầu
                        </Text>
                      </TouchableOpacity>
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.pinnedBannerDropdownText,
                            { color: "#f44336" },
                          ]}
                        >
                          Bỏ ghim
        ...
        <View style={styles.audioPreviewBar}>
          <View style={styles.audioPreviewInfo}>
            ...
            <Text style={styles.audioPreviewText}>
              Tin nhắn thoại ({formatRecordTime(recordingTime)})
            </Text>
          </View>
          <TouchableOpacity
            onPress={cancelRecording}
            style={styles.audioPreviewCancelBtn}
          >
            <Ionicons name="close" size={20} color="#888" />
          </TouchableOpacity>
          <TouchableOpacity
            ...
            style={[
              styles.audioPreviewSendBtn,
              isUploading && styles.audioPreviewSendBtnDisabled,
            ]}
          >
            <Text style={styles.audioPreviewSendText}>
              {isUploading ? "..." : "Gửi"}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      {showMentionDropdown && selectedChat?.type === "group" && (
        <View style={styles.mentionDropdown}>
          ...
          <ScrollView horizontal={false} keyboardShouldPersistTaps="always">
            {/* Option @all */}
            {(mentionSearch === "" ||
              "tất cả".includes(mentionSearch.toLowerCase()) ||
            ...
            {selectedMessage?.messageID && (
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  const isGroup = selectedChat.type === "group";
                  const pinEvent = isGroup
                    ? "ghim_group_message"
                    : "ghim_message";
                  const unpinEvent = isGroup
                    ? "unghim_group_message"
                    : "unghim_message";
                  const payload = isGroup
                    ? {
                  ...
                  if (selectedMessage.pinnedInfo) {
                    ...
                    if (isGroup && !canPinMessages) {
                      Alert.alert(
                        "Thông báo",
                        "Chỉ trưởng nhóm và phó nhóm mới có thể bỏ ghim tin nhắn",
                      );
                      setShowMenu(false);
                      return;
                    }
                    ...
                    if (isGroup && !canPinMessages) {
                      Alert.alert(
                        "Thông báo",
                        "Chỉ trưởng nhóm và phó nhóm mới có thể ghim tin nhắn",
                      );
                      setShowMenu(false);
                      return;
                    }
                    ...
                    if (pinnedCount >= 3) {
                      Alert.alert(
                        "Thông báo",
                        "Chỉ có thể ghim tối đa 3 tin nhắn",
                      );
                      setShowMenu(false);
                      return;
                    }
          ...
          >
            ...
            <ScrollView style={styles.infoPanelContent}>
              ...
              <View style={styles.infoPanelSection}>
                <Text style={styles.infoPanelSectionTitle}>
                  Nhắc hẹn sắp tới
                </Text>
                <View style={styles.infoPanelEmptyBox}>
                  <Text style={styles.infoPanelEmptyText}>
                ...
                {pinnedMessages.length === 0 ? (
                  ...
                  <View style={styles.pinnedListInPanel}>
                    {pinnedMessages.map((item, idx) => (
                      <View
                        key={item.messageID || item.tempID}
                        style={styles.pinnedItemInPanel}
                      >
                        <TouchableOpacity
                        ...
                        >
                          ...
                          <View style={styles.pinnedItemTextContainer}>
                            ...
                            <Text
                              style={styles.pinnedItemContent2}
                              numberOfLines={2}
                            >
                              {item.content || "Media"}
                            </Text>
                          ...
                          >
                            ...
                            >
                              <Ionicons
                                ...
                                color="#333"
                              />
                              <Text style={styles.pinnedItemDropdownText}>
                                Đưa lên đầu
                              </Text>
                            </TouchableOpacity>
        ...
        <View style={styles.forwardModalOverlay}>
          <View style={styles.forwardModalContainer}>
            ...
            <FlatList
              ...
              keyExtractor={(item) => item.chatID}
              renderItem={({ item }) => {
                const isSelected = selectedChatsForForward.includes(
                  item.chatID,
                );
                return (
                  <TouchableOpacity
              ...
              <TouchableOpacity
                style={[
                  ...
                  selectedChatsForForward.length === 0 &&
                    styles.forwardSubmitBtnDisabled,
                ]}
          ...
          invitedNames={incomingGroupCall.allMemberInfos
            .filter(
              (m) =>
                ...
                m.userID !== incomingGroupCall.callerID,
            )
            .map((m) => m.name)}
          onAccept={() => {
      ...
      {showGroupCall && user && groupCallData && (
        <GroupCallScreen
          ...
          members={
            groupCallData.allGroupMembers ||
            // Fallback: Lấy members từ selectedChat nếu đang mở group chat đó
      ...
      {deletingChat && (
        <Modal
          ...
          transparent
          animationType="fade"
          onRequestClose={() => setDeletingChat(null)}
        >
