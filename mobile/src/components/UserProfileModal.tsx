...
const validateName = (name: string) =>
  /^([A-ZÀ-Ỵ][a-zà-ỹ]*)(\s[A-ZÀ-Ỵ][a-zà-ỹ]*)+$/.test(
...
const UserProfileModal = ({ visible, onClose, user, setUser }: Props) => {
  ...
  const handleSave = async () => {
    ...
    if (!validateName(profile.name)) {
      setErrorMessage(
        "Tên không hợp lệ! Ít nhất 2 từ, mỗi từ bắt đầu chữ hoa.",
      );
      return;
    }
    ...
    if (pwStep === 3) {
      ...
      try {
        ...
        const res = await fetch(
          `${API_URL}/api/users/${user?.userID}/password`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
                  ...
                  >
                    <View style={styles.avatarWrap}>
                      <Image
                        source={{
                          uri:
                            profile.avatar || "https://via.placeholder.com/90",
                        }}
                        style={styles.avatar}
                      />
                      {isEditing && (
                          ...
                          >
                            ...
                            {options.map((opt) => (
                              <TouchableOpacity
                                key={opt}
                                style={
                                  (profile as any)[key] === opt
                                    ? styles.dateOptSelected
                                    : styles.dateOpt
                                }
                                onPress={() =>
                                  setProfile((p) => ({ ...p, [key]: opt }))
                                }
                              >
                                <Text
                                  style={
                                    (profile as any)[key] === opt
                                      ? styles.dateOptTextSelected
                                      : styles.dateOptText
                                  }
                                >
                                  {opt}
                      ...
                      <View style={styles.genderRow}>
                        {["Nam", "Nữ", "Khác"].map((g) => (
                          <TouchableOpacity
                            ...
                            style={[
                              styles.genderBtn,
                              profile.gender === g && styles.genderSelected,
                            ]}
                            onPress={() =>
