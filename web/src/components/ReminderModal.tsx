...
import { loadReminders, saveReminders, formatReminderDate } from '../hooks/useReminderChecker';
...
const ReminderModal = ({ chatID, onClose, onCreated, onDeleted }: Props) => {
  const [reminders, setReminders] = useState<Reminder[]>(() =>
    loadReminders().filter((r) => r.chatID === chatID)
  );
  ...
  const handleCreate = () => {
    ...
    const all = [...loadReminders(), r];
    saveReminders(all);
    const chatReminders = all.filter((x) => x.chatID === chatID);
    setReminders(chatReminders);
    onCreated(r);
  ...
  const handleDelete = (r: Reminder) => {
    ...
    saveReminders(all);
    setReminders(all.filter((x) => x.chatID === chatID));
    onDeleted(r);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-700 shrink-0">
          ...
          <span className="flex-1 text-[15px] font-bold text-gray-900 dark:text-gray-100">Nhắc hẹn</span>
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
          ...
          {(['list', 'create'] as const).map((t) => (
            <button
              ...
              className={`flex-1 py-2.5 text-[13px] font-semibold transition-colors border-b-2 ${
                tab === t ? 'text-[#0e9de8] border-[#0e9de8]' : 'text-gray-400 border-transparent hover:text-gray-600'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === 'create' && (
            <div className="px-5 py-4 flex flex-col gap-4">
              {/* Tiêu đề */}
              <div>
                <label className="text-[12px] font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block uppercase tracking-wide">Tiêu đề nhắc hẹn</label>
                <input
                  type="text"
                  value={title}
              ...
              {/* Thời gian */}
              <div>
                <label className="text-[12px] font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block uppercase tracking-wide">Thời gian</label>
                <input
                  type="datetime-local"
                  value={datetime}
              ...
              {/* Lặp lại */}
              <div>
                <label className="text-[12px] font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block uppercase tracking-wide">Lặp lại</label>
              ...
              {error && (
                <p className="text-[12px] text-red-500 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">{error}</p>
              )}
              ...
              {title && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 flex flex-col items-center gap-2 border border-gray-100 dark:border-gray-700">
                  ...
                  <p className="text-[14px] font-bold text-gray-900 dark:text-gray-100 text-center">{title}</p>
                  <p className="text-[12px] text-gray-500 flex items-center gap-1">
                    <FaClock className="text-[10px]" />
                    {formatReminderDate(datetime || new Date().toISOString())}
                  </p>
                  {repeat !== 'none' && (
                    <span className="text-[11px] bg-blue-100 dark:bg-blue-900/40 text-[#0e9de8] px-2 py-0.5 rounded-full">{repeatLabel[repeat]}</span>
                  )}
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-[#0e9de8] text-white text-[14px] font-bold hover:bg-[#0077c2] transition-colors shadow-sm"
              >
                Tạo nhắc hẹn
              </button>
            </div>
            ...
            <div className="px-5 py-4 flex flex-col gap-3">
              ...
              {upcoming.length > 0 && (
                <>
                  ...
                  {upcoming.map((r) => (
                    <ReminderCard key={r.id} reminder={r} onDelete={() => handleDelete(r)} />
                  ))}
                </>
              )}
              ...
              {past.length > 0 && (
                <>
                  ...
                  {past.map((r) => (
                    <ReminderCard key={r.id} reminder={r} onDelete={() => handleDelete(r)} isPast />
                  ))}
                </>
              )}
            </div>
...
const ReminderCard = ({ reminder, onDelete, isPast }: {
  reminder: Reminder;
  onDelete: () => void;
  isPast?: boolean;
}) => (
  <div className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
    ...
    <div className="flex-1 min-w-0">
      <p className={`text-[13px] font-semibold truncate ${isPast ? 'line-through text-gray-400' : 'text-gray-900 dark:text-gray-100'}`}>
        {reminder.title}
      </p>
