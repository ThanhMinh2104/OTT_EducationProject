...
import {
  ...
  FaVideo, FaVideoSlash, FaUsers, FaTimes, FaSearch, FaCheck,
} from 'react-icons/fa';
    ...
    <div className="relative bg-gray-800 rounded-xl overflow-hidden flex items-center justify-center w-full h-full min-h-[140px]">
      ...
      {!showVideo && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4">
          ...
          />
          {participant.status === 'ringing' && <p className="text-gray-400 text-xs animate-pulse">Đang đổ chuông...</p>}
...
const GroupCallModal = ({
  user, groupID, groupName, groupAvatar, members,
  isCallee = false, initialWithVideo = true, initialParticipants = [], onClose,
}: Props) => {
  ...
  const createPC = (remoteID: string): RTCPeerConnection => {
    ...
    pc.onicecandidate = e => {
      if (e.candidate && isActiveRef.current) {
        socket.emit('group-call-ice', { groupID, to: remoteID, from: user.userID, candidate: e.candidate });
      }
    };
    ...
    pc.ontrack = e => {
      ...
      setParticipants(prev => {
        ...
        if (exists) {
          return prev.map(p => p.userID === remoteID ? { ...p, stream, status: 'connected' } : p);
        }
  ...
  const setupSocketListeners = () => {
    ...
    socket.on('group-call-user-joined', async (data: {
      groupID: string; userID: string; userInfo: { name: string; avatar?: string };
    }) => {
      ...
      socket.emit('group-call-offer', {
        groupID, to: data.userID, from: user.userID,
        offer, userInfo: { name: user.name, avatar: user.anhDaiDien },
      });
    });

    // Nhận offer → tạo answer
    socket.on('group-call-offer', async (data: {
      groupID: string; from: string; offer: RTCSessionDescriptionInit; userInfo: { name: string; avatar?: string };
    }) => {
      if (!isActiveRef.current || data.groupID !== groupID) return;

      setParticipants(prev => {
        if (prev.find(p => p.userID === data.from)) return prev;
        return [...prev, { userID: data.from, name: data.userInfo.name, avatar: data.userInfo.avatar, status: 'connected' as const }];
      });
    ...
    socket.on('group-call-answer', async (data: {
      groupID: string; from: string; answer: RTCSessionDescriptionInit;
    }) => {
      if (!isActiveRef.current || data.groupID !== groupID) return;
    ...
    // ICE candidate
    socket.on('group-call-ice', async (data: {
      groupID: string; from: string; candidate: RTCIceCandidateInit;
    }) => {
      if (!isActiveRef.current || data.groupID !== groupID) return;
      const pc = pcsRef.current.get(data.from);
    ...
    socket.on('group-call-user-rejected', (data: { groupID: string; userID: string }) => {
      ...
      setParticipants(prev => prev.map(p =>
        p.userID === data.userID ? { ...p, status: 'rejected' as const } : p
      ));
    });
    ...
    socket.on('group-call-session-info', async (data: {
      groupID: string;
      ...
      for (const p of data.participants) {
        ...
        if (user.userID < p.userID) {
          ...
          await pc.setLocalDescription(offer);
          socket.emit('group-call-offer', {
            groupID, to: p.userID, from: user.userID,
            offer, userInfo: { name: user.name, avatar: user.anhDaiDien },
          });
        }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      if (isCallee) {
        ...
        setParticipants([
          { userID: user.userID, name: user.name, avatar: user.anhDaiDien, stream, status: 'connected', isVideoOff: !initialWithVideo },
          ...otherTiles,
        ]);
        ...
        // Callee bắt đầu đếm thời gian ngay
        callStartedRef.current = true;
        timerRef.current = window.setInterval(() => setCallDuration(p => p + 1), 1000);
      }
  ...
  const startCall = async () => {
    ...
    const localP: Participant = {
      userID: user.userID, name: user.name, avatar: user.anhDaiDien,
      stream, status: 'connected', isVideoOff: false,
    };
  ...
  const toggleVideo = async () => {
    ...
    setParticipants(prev => prev.map(p =>
      p.userID === user.userID ? { ...p, isVideoOff: newOff } : p
    ));
  };

  // ── Grid ──────────────────────────────────────────────────────────────────

  const gridClass = participants.length <= 1 ? 'grid-cols-1'
  ...
  if (screen === 'select') {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
        <div className="bg-white rounded-2xl shadow-2xl w-[520px] max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <h2 className="text-lg font-semibold text-gray-900">Tạo cuộc gọi</h2>
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-500"><FaTimes /></button>
          </div>
          <div className="px-6 py-3 border-b">
            <div className="flex items-center gap-2 bg-gray-100 rounded-full px-4 py-2">
              <FaSearch className="text-gray-400 text-sm" />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm kiếm thành viên" className="flex-1 bg-transparent text-sm outline-none text-gray-700" />
            </div>
          </div>
          <div className="flex flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto px-4 py-2">
              {filtered.map(m => {
                ...
                return (
                  <div key={m.userID} onClick={() => setSelected(prev => isSel ? prev.filter(s => s.userID !== m.userID) : [...prev, m])}
                    className="flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-gray-50 cursor-pointer">
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${isSel ? 'bg-blue-500 border-blue-500' : 'border-gray-300'}`}>
                      {isSel && <FaCheck className="text-white text-[10px]" />}
                    </div>
                    <img src={m.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${m.name}`} alt={m.name} className="w-10 h-10 rounded-full object-cover" />
            ...
            {selected.length > 0 && (
              <div className="w-48 border-l px-3 py-2 overflow-y-auto">
                <p className="text-xs font-semibold text-gray-500 mb-2">Đã chọn {selected.length}/{members.length}</p>
                {selected.map(m => (
                  <div key={m.userID} className="flex items-center gap-2 py-1.5">
                    <img src={m.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${m.name}`} alt={m.name} className="w-8 h-8 rounded-full object-cover" />
                    <span className="text-xs text-gray-700 flex-1 truncate">{m.name}</span>
                    <button onClick={() => setSelected(prev => prev.filter(s => s.userID !== m.userID))} className="w-5 h-5 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center">
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t">
            <button onClick={onClose} className="px-5 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-sm font-medium text-gray-700">Hủy</button>
            <button onClick={startCall} disabled={selected.length === 0}
              className="px-6 py-2 rounded-xl bg-blue-500 hover:bg-blue-600 disabled:opacity-40 disabled:cursor-not-allowed text-sm font-semibold text-white">Gọi</button>
          </div>
        </div>
      </div>
    );
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          {groupAvatar
            ? <img src={groupAvatar} alt={groupName} className="w-7 h-7 rounded-full object-cover" />
            : <div className="w-7 h-7 rounded-full bg-blue-500 flex items-center justify-center"><FaUsers className="text-white text-xs" /></div>
          }
          <div>
      ...
      <div className="flex items-center justify-center gap-4 py-3 bg-gray-900 border-t border-white/10 shrink-0">
        ...
        <button onClick={leaveCall} title="Rời cuộc gọi"
        ...
        <button onClick={toggleVideo} title={isVideoOff ? 'Bật camera' : 'Tắt camera'}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 rounded-2xl">
          <div className="flex flex-col items-center gap-3">
            <div className="w-14 h-14 rounded-full bg-gray-700 flex items-center justify-center">
              <FaPhoneSlash className="text-gray-300 text-xl" />
            </div>
            <p className="text-white font-semibold">{endedMessage}</p>
          </div>
        </div>
      )}
    </div>
