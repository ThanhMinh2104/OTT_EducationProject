...
import { FaUser, FaSignOutAlt, FaComments, FaCog, FaShieldAlt, FaMobileAlt, FaUserSlash, FaHistory } from 'react-icons/fa';
...
const Sidebar = ({ user, setUser }: Props) => {
  ...
  const [settingsTab, setSettingsTab] = useState<'account' | 'devices' | 'history' | 'deactivate'>('account');
  ...
  return (
    <>
      ...
      {/* Settings Modal */}
      {showSettingsModal && (
        <div className="fixed inset-0 z-1000 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowSettingsModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden animate-fade-slide-in" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-colors ${
                      settingsTab === 'account' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => { setSettingsTab('devices'); fetchSessions(); }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-colors ${
                      settingsTab === 'devices' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => { setSettingsTab('history'); fetchLoginHistory(); }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-colors ${
                      settingsTab === 'history' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-colors ${
                      settingsTab === 'deactivate' ? 'bg-red-50 text-red-600' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 bg-gray-50 rounded-xl">
                        ...
                        <button
                          onClick={() => { setShowSettingsModal(false); openProfile(); }}
                      ...
                      <div className="p-4 border border-gray-200 rounded-xl">
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-gray-800">{user?.gioTinh || 'Chưa cập nhật'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex items-start gap-3">
                                <div className="text-3xl">
                                  {session.deviceType === 'mobile' ? '📱' : session.deviceType === 'desktop' ? '💻' : '🌐'}
                                </div>
                                <div>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex items-start gap-3">
                                <div className="text-2xl">
                                  {item.deviceType === 'mobile' ? '📱' : item.deviceType === 'desktop' ? '💻' : '🌐'}
                                </div>
                                <div className="flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full ${
                                      ...
                                      {item.status === 'active' ? 'Đang hoạt động' : item.status === 'logged_out' ? 'Đã đăng xuất' : 'Hết hạn'}
                                    </span>
                                  </div>
